...
public:
  GymVis() : Node("gym_vis"), count_(0) {
    car_publisher_ = this->create_publisher<visualization_msgs::msg::Marker>(
        "f1tenth_car", 10);
    lidar_publisher_ = this->create_publisher<visualization_msgs::msg::Marker>(
        "f1tenth_lidar_data", 10);
    // traj_publisher_ =
    // this->create_publisher<visualization_msgs::msg::Marker>("f1tenth_traj",
    // 10);

    gym_car_subscriber_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/ego_racecar/odom", 5,
        std::bind(&GymVis::car_callback, this, std::placeholders::_1));
    gym_lidar_subscriber_ =
        this->create_subscription<sensor_msgs::msg::LaserScan>(
            "/scan", 5,
            std::bind(&GymVis::lidar_callback, this, std::placeholders::_1));
    drive_publisher_ =
        this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>(
            "/drive", 10);

    publisher_timer_ = this->create_wall_timer(
        15ms, std::bind(&GymVis::publisher_timer_callback, this));
...
void GymVis::car_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
  ...
  car_marker.pose.position.z =
      msg->pose.pose.position.z + car_marker.scale.z / 2;
  ...
  // DEBUG MESSAGES
  //  RCLCPP_INFO(this->get_logger(), "Position: [%.2f, %.2f, %.2f] Orientation:
  //  [%.2f, %.2f, %.2f, %.2f]",
...
void GymVis::lidar_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    if (std::isfinite(range) && range >= msg->range_min &&
      ...
      // Convert polar coordinates (range, angle) to Cartesian coordinates (x,
      // y)
      ...
      point_in_source.point.z =
          0.0; // Laser scan usually provides 2D data (in the XY plane)
...
void GymVis::car_marker_setup(std::string header, float scale[3],
                              float color[4]) {
  ...
  // Define the size of the rectangle (e.g., 2 meters long, 1 meter wide, 0.5
  // meter high)
...
void GymVis::lidar_marker_setup(std::string header, float scale[3],
                                float color[4]) {
  ...
  lidar_scan_markers.scale.x =
      scale[0]; // Width of points (smaller is more visible)
