    ...
    : Node("state_manager_node")
{
    ...
    lattice_viz_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lattice_viz", 10);
...
void StateManagerNode::loadRacingLine() {
    ...
    if (racing_line_.empty()) {
        RCLCPP_ERROR(this->get_logger(), "Failed to load racing line from: %s",
    ...
    const std::string& filename)
{
    ...
    if (!file.is_open()) {
        RCLCPP_ERROR(this->get_logger(), "Failed to open racing line file: %s",
                     filename.c_str());
        return points;
    }
