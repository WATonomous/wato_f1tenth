...
try
{
  ...
  result->path = pathToRosPath(plan.path, odom);
...
nav_msgs::msg::Path PlannerNode::pathToRosPath(
  ...
  const local_planning::Odometry & odom)
{
  nav_msgs::msg::Path ros_path;
  ros_path.header.frame_id = "base_link";
  ros_path.header.stamp = this->now();

  const double cos_heading = std::cos(odom.heading);
  const double sin_heading = std::sin(odom.heading);

  for (const auto & p : path) {
    const double dx = p.x - odom.position.x;
    const double dy = p.y - odom.position.y;

    geometry_msgs::msg::PoseStamped pose;
    pose.header = ros_path.header;
    pose.pose.position.x = cos_heading * dx + sin_heading * dy;
    pose.pose.position.y = -sin_heading * dx + cos_heading * dy;
