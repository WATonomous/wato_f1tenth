...
private:
  ...
  void publishGrid(const sensor_msgs::msg::LaserScan & scan)
  {
    ...
    const std::string scan_frame_id =
      scan.header.frame_id.empty() ? laser_frame_id_ : scan.header.frame_id;
  ...
  void markInflatedObstacle(nav_msgs::msg::OccupancyGrid & grid, int center_col, int center_row)
  {
    const int inflation_cells =
      static_cast<int>(std::ceil(obstacle_inflation_radius_ / resolution_));
