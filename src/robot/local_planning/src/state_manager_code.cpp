    ...
    const Point& ego_position)
{
    ...
    for (int step = 1; accumulated < SCAN_DISTANCE; ++step) {
        ...
        int col = static_cast<int>((racing_line_[curr].x - occupancy_grid.origin.x) / occupancy_grid.resolution);
        int row = static_cast<int>((racing_line_[curr].y - occupancy_grid.origin.y) / occupancy_grid.resolution);
