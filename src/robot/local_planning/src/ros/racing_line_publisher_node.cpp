...
{
// The only place the raceline CSV format is parsed.  Anything else re-reading
// it is a second ingestion route that silently diverges the day the format
// changes; test harnesses consume a recorded /racing_line message instead.
//
// Topic contract: speed rides in pose.position.z.  See ros_adapters.hpp.
class RacingLinePublisherNode : public rclcpp::Node
{
public:
  RacingLinePublisherNode()
  : Node("racing_line_publisher_node")
  {
    ...
    publisher_ = create_publisher<nav_msgs::msg::Path>(
      get_parameter("racing_line_topic").as_string(),
    ...
    while (std::getline(file, line)) {
      ...
      try {
        ...
        pose.pose.position.x = std::stod(x); pose.pose.position.y = std::stod(y);
        pose.pose.position.z = std::stod(velocity);
        path_.poses.push_back(std::move(pose));
      } catch (const std::exception &) {
        RCLCPP_WARN(get_logger(), "Skipping invalid CSV row: %s", line.c_str());
      }
    }
    if (path_.poses.empty()) {
      throw std::runtime_error("Racing-line CSV contains no valid waypoints");
    }
    publish();
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<local_planning::RacingLinePublisherNode>());
