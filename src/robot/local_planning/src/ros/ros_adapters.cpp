  ...
  const std::string & frame_id)
{
  ...
  out.proposed_intent = static_cast<uint8_t>(data.proposed_intent);
  out.executed_intent = static_cast<uint8_t>(data.executed_intent);
  out.transition_class = static_cast<uint8_t>(data.transition_class);
  out.transition_reason = static_cast<uint8_t>(data.transition_reason);
  out.recovery_reason = static_cast<uint8_t>(data.recovery_reason);
  out.pending_transition_s = data.pending_transition_s;
  out.pending_grid_count = data.pending_grid_count;
  out.merge_probe_valid_cycles = data.merge_probe_valid_cycles;
  out.merge_probe_available = data.merge_probe_available;
  out.costmap_sequence = data.costmap_sequence;
  out.costmap_stamp_s = data.costmap_stamp_s;
  out.opponent_observation_sequence = data.opponent_observation_sequence;
